...
@onready var anim: AnimatedSprite2D = $anim_npc2  # Nome diferente para animação
...
func _ready() -> void:
	...
	if anim:
		...
		print("Erro: AnimatedSprite2D 'anim_npc2' não encontrado!")
	if texture:
		texture.hide()
	else:
	...
	print("NPC2 inicializado - Diálogos iniciais: ", initial_lines)
...
func _on_area_2d_body_entered(body: Node) -> void:
	if body.is_in_group("player"):
		if texture:
			texture.show()
		print("Jogador entrou na área do NPC2 - Texture visível")

func _on_area_2d_body_exited(body: Node) -> void:
	if body.is_in_group("player"):
		if texture:
			texture.hide()
		print("Jogador saiu da área do NPC2 - Texture escondida")
...
func _unhandled_input(event):
	...
	for body in bodies:
		if body.is_in_group("player"):
			in_area = true
			break
	print("Evento de entrada NPC2 - Pressionado: ", event.is_action_pressed("interact"), " | Na área: ", in_area, " | Diálogo ativo: ", DialogManager.is_message_active)
	if not in_area or not event.is_action_pressed("interact"):
		return
	if not DialogManager.is_message_active:
		print("NPC2 interagido - Iniciando diálogo com linhas: ", current_lines)
		if texture:
			texture.hide()
		if message_sound:
